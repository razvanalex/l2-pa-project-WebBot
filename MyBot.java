...
//        }
        ...
        if (effectiveness <= 2.0 / 3.0 * (ourShips.size() + 1)) {
            ...
            if (!inDangerPlanets.isEmpty()) {
                ...
                if (inDangerPlanets.isEmpty()) {
                    ...
                    /** sortare ships */
                    List<SortShipsClass.MyEntry> shipMoves;
                    List<Planet> planetsTarget = new ArrayList<>(noOnePlanets);
                    planetsTarget.addAll(enemyPlanets);
                    if (planetsTarget.isEmpty() == false) {
                        shipMoves = SortShipsClass.sortShips(ships, planetsTarget);
    ...
    public static double planetInDangerScore(final int crtPlayeID, final Entity planet,
    ...
    public static TreeMap<Double, Ship> nearbyEnemyShips(final Ship entity, final List<Ship> enemyShips,
                                                         GameMap gameMap, boolean dockedShips) {
    ...
    * */
//    public static double minimax(int crtPlayer, GameMap crtState, int depth, int numOurShips, int numEnemyShips,
//                                 int numOurPlanets, int numEnemyPlanets) {
//        if (crtState.gameOver() || depth == 0) {
//            return overallScore(numOurShips, numEnemyShips, numOurPlanets, numEnemyPlanets);
//        }
//
...
//    }
    ...
    public static void attackShipsNearPlanet(final Planet planet, final List<Ship> ourShips,
        ...
        for (Ship s : ourShips) {
            ...
            TreeMap<Double, Ship> map = nearbyEnemyShips(s, enemyShips, gameMap, true);
            Map.Entry<Double, Ship> nextTarget = map.pollLastEntry();

            if (s.getTarget() == null) {
                s.setTarget(nextTarget.getValue());
            }
            boolean avoidObstacles = true;
            ...
            final ThrustMove newThrustMove = Navigation.navigateShipToDock(gameMap,
                    s, s.getTarget(), Constants.MAX_SPEED, avoidObstacles);
