...
auto Halite::count_conquered_planets(unsigned int& free_planets,
                                     unsigned int& occupied_planets,
                                     unsigned int& destroyed_planets) -> void {
...
GameStatistics Halite::run_game(std::vector<std::string>* names_,
    ...
    try {
        while (!game_complete()) {
            bool collision_status = self_collisions;
            ...
            if (collision_status != self_collisions) {
                std::cerr << "Collision detected, turn " << turn_number << ".\n";
            }
        }
    }
    ...
    unsigned int free_planets, occupied_planets, destroyed_planets;
    count_conquered_planets(free_planets,
                            occupied_planets,
                            destroyed_planets);
    ...
    if (enable_replay) {
        ...
        if (turn_number <= 1 && !quiet_output && error_tags.size() > 0) {
            ...
            Replay replay = {
                ...
                free_planets, occupied_planets, destroyed_planets, self_collisions
            };
            ...
            stats.output_filename = replay_directory + filename + ".json";
            replay.output(stats.output_filename, false);
