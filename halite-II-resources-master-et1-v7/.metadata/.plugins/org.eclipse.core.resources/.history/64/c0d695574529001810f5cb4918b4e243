...
public class Ship extends Entity {
            ...
            final int maxThrust)
    {
        ...
        System.out.print("aici1 ");
        while(initial.getDistanceTo(targetPos)>ship.getDistanceTo(targetPos)/2){
        	...
        	dist+=nextPosition.getDistanceTo(initial);
        	initial=nextPosition;
        }
        System.out.print("aici2 ");
        return dist;
        }
            ...
            final double angularStepRad)
    {
        ...
        final double newTargetDx = Math.cos(angleRad) * thrust;
        final double newTargetDy = Math.sin(angleRad) * thrust;
